...
import Model.Date;
import Model.Date.Date_Debut_Reservation;
import Model.Date.Date_nonorganiser;
import Model.Date.Date_nonvalid;
import Model.Date.Date_syntaxe;
import Model.Date.DiffSup365;
import Model.Hotel;
...
public class Table_Reseravtion_User extends javax.swing.JFrame {
    ...
    private void initComponents(String dated, String datef) {
        ...
        roompricebox.setModel(new javax.swing.DefaultComboBoxModel<>(
                new String[] { "SONA", "TERASSE", "VUESURMERE", "VUESURFORET" }));
        // le positionement exact du combobox:
    ...
    public int cancelReservationBtnActionPerformed(java.awt.event.ActionEvent evt) throws Exception {
        ...
        if (i >= 0) {
            ...
            if (Date.comparer(Date.Recupere_date(startdatetext.getText()), today) == 1) {
                ...
                JOptionPane.showMessageDialog(frame, "Vous ne pouvez pas annuler cette réservation !", "Erreur",
                        JOptionPane.ERROR_MESSAGE);
                return -1;
            }
        } else {
            JOptionPane.showMessageDialog(frame, "Veuillez sélectionner une réservation à annuler.", "Infornation",
    ...
    public int addreservationbtnActionPerformed(java.awt.event.ActionEvent evt) throws Exception {
        try {
            ...
            Model.addRow(new Object[] { Reservation.getNb() + 1, roomtypebox.getSelectedItem(),
                    roompricebox.getSelectedItem(), Control.total_prix + "$", startdatetext.getText(),
    ...
    public int updatebtnActionPerformed(java.awt.event.ActionEvent evt) throws Exception {
        ...
        if (i >= 0) {
            ...
            JOptionPane.showMessageDialog(frame, "Veuillez sélectionner une ligne à mettre à jour.", "Erreur",
                    JOptionPane.ERROR_MESSAGE);
        }
        return -1;
